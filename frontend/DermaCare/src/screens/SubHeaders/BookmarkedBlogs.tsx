...
const BookmarkedBlogs: React.FC<BookmarkedBlogsScreenProps> = () => {
  ...
  useEffect(() => {
    fetch(`${API_HOME}/api/blogs/`)
      ...
      .then((data) => {
        const bookmarkedBlogs = (data.blogs || []).filter(
          (blog: Blog) => blog.bookmarked === true
        );
        setBlogs(bookmarkedBlogs);
        setLoading(false);
      })
  ...
  const toggleBookmark = async (id: string, currentStatus: boolean) => {
    try {
      const response = await fetch(`${API_HOME}/api/blogs/${id}/`, {
        method: 'PATCH',
        headers: {
      ...
      setBlogs((prevBlogs) =>
        prevBlogs
          .map((blog) =>
            blog.id === id ? { ...blog, bookmarked: !currentStatus } : blog
          )
          .filter((blog) => blog.bookmarked) // Remove if unbookmarked
      );
    } catch (error) {
      console.error('Bookmark update error:', error);
      setError('Failed to update bookmark. Try again.');
    }
  };
  ...
  const renderItem = ({ item }: { item: Blog }) => (
    <View style={styles.blogItem}>
      ...
      <TouchableOpacity
        style={styles.readMoreButton}
        onPress={() =>
          handlePress(item.title, item.image_url, item.subText, item.body)
        }
      >
        <Text style={styles.readMoreText}>Read More</Text>
      </TouchableOpacity>
      ...
      <TouchableOpacity
        ...
        onPress={() => toggleBookmark(item.id, item.bookmarked ?? true)}
      >
        <Text style={styles.bookmarkButtonText}>Unbookmark</Text>
      </TouchableOpacity>
    </View>
  );
