...
//     try {
//       console.log('Fetching cart count with healthCareNumber:', healthCardNumber);
//       const response = await fetch(`${API_HOME}/api/get-cart-quantity/`, {
//         method: 'POST',
//         headers: {
//           'Content-Type': 'application/json',
//         },
//         body: JSON.stringify({
//           healthCareNumber: healthCardNumber, // Match backend key
//         }),
//       });
...
//       setCartItems(data.count || "0"); // Adjust based on backend response
//     } catch (error) {
...
//   // useEffect(() => {
//   //   if (healthCardNumber) {
//   //     fetchCartCount(); // Call only when healthCardNumber is set
//   //   }
//   // }, [healthCardNumber]);

//   const handleCartPress = () => {
//     navigation.navigate('Cart', { healthCardNumber });
//   };

//   if (loading) {
//     return (
//       <View style={styles.loadingContainer}>
//         <ActivityIndicator size="large" color="#016C9D" />
//       </View>
//     );
//   }
...
//           <Text style={styles.title}>Serums</Text>
//           <FlatList
//             data={products.filter((product) => product.type === 'Serum')}
//             keyExtractor={(item) => item.id}
//             horizontal
...
const Products = () => {
  ...
  // Fetch cart count once healthCardNumber is available
  useEffect(() => {
    if (healthCardNumber) {
      fetchCartCount();
    }
  ...
  const fetchCartCount = async () => {
    try {
      const response = await fetch(`${API_HOME}/api/get-cart-quantity/`, {
        ...
        body: JSON.stringify({ healthCardNumber: healthCardNumber }),
      });
      ...
      const data = await response.json();
      setCartItems(data.count?.toString() || '0');
    } catch (error) {
      console.error('Error fetching cart count:', error);
      setCartItems('0');
    }
